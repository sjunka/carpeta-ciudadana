...
function documento(img, ev) {
  ...
  return [
    ...
    tabla(['ID', 'Microservicio', 'Responsabilidad', 'Dominio · veredicto', 'Historias', 'Estado'],
      ms.filas.map((m) => [m.id, m.nombre, m.responsabilidad, `${m.dominio} · ${m.veredicto}`, m.historias, m.nota ? `${m.estado}. ${m.nota}` : m.estado])),
    ...
    h1('dep'), md(dep.intro), ...dep.diagramas.map((d) => figura(d)),
